...
if __name__ == '__main__':
    ...
    for ifold in range(5):
        ...
        test_idxs = np.where(df['fold'] != 6)[0]
        ...
        for i in range(1, 4):
            for index, row in df.iloc[test_idxs].iterrows():
                ...
                print(feature.shape)
                os.makedirs(os.path.join(f'./NodeFeature_pretrainNew/nodeEmbed_formLSTMpretrain/{ifold}', f'{i}Multiple', str(name)),
                            exist_ok=True)
                np.save(os.path.join(f'./NodeFeature_pretrainNew/nodeEmbed_formLSTMpretrain/{ifold}', f'{i}Multiple', str(name),
                                     f'feature_{hiddennum}.npy'), feature)